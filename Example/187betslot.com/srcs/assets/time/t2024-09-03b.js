...
function sppl(AA, AB) {
    ...
    if (AC == -1) return AB
    var AD = AA.slice(-1),
        AE = AB.substr(AC + 2, AB.indexOf(']') - AC - 2).split('|'),
        AF = AB.slice(0, AC)
    if (l == 'cs') {
        if (AA != 0 && AA < 5) return AF + AE[0];
        return AF + AE[1]
    }
    if (l == 'mk') {
        if (AA % 10 == 1 && AA % 100 != 11) return AF + AE[1];
        return AF + AE[0]
    }
...
function nicetime(AG, AH) {
    var AI = '',
        AJ = Math.floor(AG / 86400),
        AK = AG % 86400
    if (AJ == 1) AI += '1 ' + p_d
    ...
    if (AI != '') AI += p_and
    AJ = (AK % 60).toFixed(AH)
    if (AH == 0 && AJ == 1) AI += '1' + p_s;
    else AI += sppl(AJ, "%n" + p_ss)
    return AI
}

function format_time(t, d, of, AL) {
    if (typeof of === U) of = 0
    var t2 = new Date(t.getTime() + of * 60000),
        f = conf['t'],
        H = t2.getUTCHours(),
        h = H,
        mins = t2.getUTCMinutes(),
        s = t2.getUTCSeconds(),
        m = l0(t2.getUTCMilliseconds()),
        a = 'am',
        IT = (H * 3600 + mins * 60 + s) / 86.4
    m = m > 99 ? m : "0" + m
    if (11 < h) {
        a = 'pm';
        h -= 12
...
function build_date(AM, AN) {
    var AO = AM.getUTCFullYear() + '',
        AP = AM.getUTCMonth() + 1
    var AQ = new Date(Date.UTC(AO, 0, 1)),
        AR = AQ.getUTCDay() - 1
    if (AR == -1) AR = 6
    var AS = Math.floor((AM - AQ) / 604800000 + (AR / 7)),
        AT = p_d + ' ' + Math.floor((AM - AQ) / 86400000 + 1)
    if (AR < 4) AS++
    if (AS == 0) {
        AS = 52
    ...
    if (p_wn == 'hy') {
        if (AS == 1) pwn = '1-ին շաբաթ';
        else pwn = AS + '-րդ շաբաթ'
    } else pwn = p_wn.replace('%n', AS)
...
function TimeIs(AU, AV, AW, AX, AY) {
    var AZ = this,
        AU = gob(AU),
        AV = gob(AV),
        AW = gob(AW),
    ...
    this.initClock = function() {
        T = new Date()
        var czo = -T.getTimezoneOffset(),
            AG = Math.abs(tD / 1000).toFixed(1),
            BG = '+',
            BH, BI, BJ = p_ss
        ...
        if (4000 < Math.abs(cY)) {
            BH = p_failh
            BI = p_failm + BK.replace('%t', '<span class="nw">' + BG + sppl((-tD / 1000).toFixed(3), "%n" + BJ) + '</span>').replace('%D', sppl((cY / 2000).toFixed(3), "%n" + BJ))
        } else if (AG < .2) {
            BH = p_exactt
            BI = BK.replace('%t', '<span class="nw">' + BG + sppl((-tD / 1000).toFixed(3), "%n" + BJ) + '</span>').replace('%D', sppl((cY / 2000).toFixed(3), "%n" + BJ))
        } else {
            var AI = nicetime(AG, 1),
                BL = p_acc
    ...
    this.tick = function(id, tack) {
        try {
            ...
            prevTS = nowTS
            if ((nextSyncT != 0 && nextSyncT < T) || T < prevT) {
                prevT = 0;
                syncn = 0;
                rsy = 1;
                httpSync()
            ...
            if (this.prevS != secs) {
                if (auSt == 1 && 3 < ticks) {
                    ...
                    if (!isNaN(BP.duration) && (BP.duration !== 0)) {
                        BP.currentTime = 0;
                        BP.volume = 1
                    }
                    BP.play()
            ...
            if (destT != 0) {
                diff = Math.floor(T.getTime() / 1000) - destT - Loffset
                if (diff < 0) {
                    var diffmsg = p_time_remaining;
                    diff = Math.abs(diff)
            ...
            if (uT && Tstate.current_page !== 'calendar') {
                var BR = BO.replace('<span class="ampm">', '').replace('</span>', '')
    ...
    this.update_favs = function(T, BT) {
        if (0 == locs['favs'].length) return ''
        var html = '',
            i
        for (i in locs['favs']) {
            if (i !== 'indexOf') {
                ...
                if (BT) {
                    ...
                    html += BW + '" id="time-' + i + '">' + BX + '<br>' + BV + '</a></li>'
                } else {
                    o = gob('time-' + i)
    ...
    this.set_susdiv = function(sdn) {
        if (sdn != T_I.susdivname) {
            T_I.take_chosen(T_I.susdivname, 0);
            T_I.susdivname = sdn;
            T_I.susdivo = gob(sdn)
        }
    }
        ...
        if (su.length !== 0)
            for (var i in su) {
                ...
                if (2 < su[i].length) {
                    ...
                    for (j = 0; j < 4; j++) {
                        su[i][j] = su[i][j].replace(/\$6/g, su[i][7][2]).replace(/\$5/g, su[i][7][1]).replace(/\$4/g, su[i][7][0]).replace(/\$2/g, su[i][2]).replace(/\$1/g, su[i][1]).replace(/\$/g, su[i][0]);
                        if (su[i][j] == '') su[i][j] = su[i][0]
                    }
                    su[i][2] = su[i][2].replace(/ /g, '_')
                    ch = '';
                    if (i == chosen_sus[sdn]) ch = '" class="chosen'
                    ...
                    slist += '<tr valign="top" id="' + sdn + 's' + i + ch + '">' + td2r + '<td><a href="' + susdest + su[i][2] + susdestquery + '" onclick="return T_I.susgo(\'' + jsesc(su[i][0]) + '\',\'' + jsesc(su[i][1]) + '\',\'' + jsesc(su[i][2]) + '\',\'' + jsesc(su[i][3]) + '\')" onmouseover="T_I.choose_sus(\'' + sdn + '\',' + i + ')"><span>' + su[i][1] + '</span></a></td>' + td2l + '</tr>'
                }
            } else {
                td2l = '<td class="t">&nbsp;</td>'
                if (ltr != 1) {
    ...
    this.take_chosen = function(sd, k) {
        if (sd != 'susdiv' && sd != 'susdivB') {
            var sn = chosen_sus[sd],
                q = current_q[sd],
                W = sus[q]
            if (k == 13 && q == '') {
                submission = E
...
function setsizes(f) {
    ww = D.body.offsetWidth ? D.body.offsetWidth : window.innerWidth ? window.innerWidth : 1
    if (D.body.offsetWidth && window.innerWidth) {
        ww = Math.min(D.body.offsetWidth, window.innerWidth)
    }
    ...
    if (wh < 400 && typeof freestar !== U) freestar.queue.push(function() {
...
function CL(CS, CT, CU, CV, CW) {
    var CX = Math.round(CS * CT * .99)
    if (CU < CX) CX = CU
...
function clock(CE, DA, DB, AV, AW, DC, AX, DD, DE, DF, DG) {
    this.timeDiv = gob(DB)
    this.dateDiv = gob(AV)
    this.dayDiv = gob(AW)
    this.sunDiv = gob(DC)
    ...
    this.locName = ''
    this.id = DG
    this.setLoc = function(CE, DH, BT) {
        var DI = locs[CE].length,
            DJ, n = 0
        DH = (DH + DI + n) % (DI + n)
        DJ = locs[CE][DH]
...
function tl_a() {
    if (auSt == 0) {
        try {
            if ((!!D.createElement('audio').canPlayType) && (-1 === navigator.userAgent.indexOf('MSIE'))) {
                if (t_au1m === 0) {
                    t_au1s = new Audio()
                    var audiotype = '.mp3'
...
function sb_o(k, x) {
    if (x) {
        if (s_on != k) {
            ...
            if (k == 'm') {
                h = 380;
                gob('sender_name').focus()
            }
            gob('social_div').style.height = h + 'px'
            s_on = k
            if (f) get_sb(k)
        }
    } else {
...
function hv_sb(k, y) {
    if (s_on !== k) Zpos(gob('btn_' + k), rg[k], y)
}
...
function t_dark() {
    var c = bod.className
    if (-1 != c.indexOf(' d ')) {
        bod.className = c.replace(' d ', ' l ')
        if (conf['c'] == 1) conf['c'] = 0
    } else {
        bod.className = c.replace(' l ', ' d ')
...
function get_zone_offset(DO, T) {
    if (typeof zones[DO][2] == 'string') DO = zones[DO][2]
    if (zones[DO].length == 3) {
        zones[DO][3] = zones[DO][2][0]
        zones[DO][4] = 1
    }
    while (zones[DO][4] < zones[DO][2].length && zones[DO][2][zones[DO][4]] <= T.getTime() / 1000) {
        zones[DO][3] = zones[DO][2][zones[DO][4] + 1]
        zones[DO][4] += 2
        sus = []
        chosen_sus = []
        prevsustime = []
...
function update_big_clock(BQ, DP, DQ) {
    return true;
    ww = D.body.offsetWidth ? D.body.offsetWidth : window.innerWidth ? window.innerWidth : 1
    if (D.body.offsetWidth && window.innerWidth) {
        ww = Math.min(D.body.offsetWidth, window.innerWidth)
    ...
    DP.t = BN
    var h = BN.getUTCHours(),
        H = h,
        DT = 'am',
        secs = BN.getUTCSeconds(),
        ms = l0(BN.getUTCMilliseconds());
    ms = ms > 99 ? ms + '' : "0" + ms
    DP.hours = h
    ...
    if (DQ || today_in_chosen_tz.getTime() != DP.prevDate) {
        ...
        if (DP.prevDate != 0) {
            if (DP.sunDiv && typeof ss[DV] != U) DP.sunDiv.innerHTML = ss[DV]
            else gob('locw').innerHTML = ''
        }
        DP.dateDiv.innerHTML = DU
        displayed_date = DU
        if (typeof spdays[DV] != U && spdays[DV] != '') {
            DP.dayDiv.innerHTML = spdays[DV]
...
function spanwrap_digitz(s, c) {
    if (c === undefined) c = ' class="mon"'
    var i = 0,
        r = ''
    s += r
...
function force_monospace(s, a, m) {
    ...
    return {
        w: w,
        margin: ml,
        totalwidth: Math.ceil(t * w + (l - 1) * ml),
        html: r.replace(/w1/g, w).replace(/w0\.6/g, Math.floor(w * .6)).replace(/w0\.8/g, Math.floor(w * .8)).replace(/-m/g, ml)
    }
}
...
function set_clock_display(o, s, ms) {
    ...
    var fontwidthfactor = {
        ...
        w = ww,
        r, fs, fwf = fontwidthfactor[conf['f']]
...
function update_changed_digits(s) {
    return true;
    var l = s.length,
        i = 0,
        c, n = 1
    for (; i < l; i++) {
        c = s.substr(i, 1)
        if (-1 < ':. '.indexOf(c)) {} else if (-1 !== 'TH'.indexOf(c)) {} else {
            w = 1
            o = gob('bcdigit' + n)
...
function nice_approx_time(n, t, g, DW) {
    d = Math.round(n - t)
    var a = Math.abs(d),
        h = '',
        i = 0,
...
function make_nice_zone_diff_sentence(DX, DY, DZ, EA) {
    var EB = DY - EA,
        EC, ED, EE
    if (EB == 0)
        EC = p_zone_diff_same
    else {
        EF = Math.floor(Math.abs(EB / 60))
        if (EF == 1) ED = 1 + p_h
...
function toggle_menu(x) {
    ...
    if (c.indexOf('menu_open') != -1) {
        ...
        var html = document.documentElement,
            pageheight = Math.max(bod.scrollHeight, bod.offsetHeight, html.clientHeight, html.scrollHeight, html.offsetHeight)
        if (Tstate.current_page !== 'just_time' && pageheight - mp.offsetHeight < wh) {
            c += ' shortbody';
            shortbody = E
        } else c += ' tallbody'
        c = c + ' menu_open'
...
function close_menu() {
    bod.className = bod.className.replace(' menu_closing', '');
    gob('menupositioner').style.height = 'auto'
}
...
function removeclass(i, c) {
    gob(i).className = gob(i).className.replace(' ' + c, '')
}
var auf = ['1s', '59s', '1m'],
    t_au1s = 0,
    t_au59s = 0,
...
function update_colors_depending_on_sun(BT) {
    conf['c'] = conf['c'] * 1
    var arr, k
    if (conf['c'] == 2) {
        if (locs['favs'].length < 1) return
        arr = 'favs'
        i = conf['h']
    } else if (conf['c'] == 3) {
    ...
    var BN = new Date(T.getTime() + get_zone_offset(locs[arr][i][1], T) * 60000),
        DV = l0(BN.getUTCMonth() + 1) + '' + l0(BN.getUTCDate())
    if (typeof EG[arr + i] === U) EG[arr + i] = []
    if (BT || typeof EG[arr + i].prevDate == U || DV != EG[arr + i].prevDate) {
        EG[arr + i].suntimes = update_suntimes(locs[arr][i], T, 0)
        EG[arr + i].prevDate = DV
    }
    if (!EG[arr + i].suntimes) return
    var CC = 0
    if (EG[arr + i].suntimes.sunrise < T && T < EG[arr + i].suntimes.sunset) CC = 1
    if (Tstate.current_page == 'main' && 1 < conf['c'] * 1 && (BT || typeof EG[arr + i].sunclass == U || EG[arr + i].sunclass !== CC)) {
        toggle('bdy', ' d ', ' l ', CC)
        EG[arr + i].sunclass = CC
    }
}

function place_badges() {
    var c = '',
        o = gob('follow_app');
    if (o) {
        if (100 < o.offsetTop) c = 'below';
        o.className = c
    }
}
...
function setcol(v) {
    conf['c'] = v;
    if (v == 0) bod.className = bod.className.replace(' d ', ' l ');
    else bod.className = bod.className.replace(' l ', ' d ');
    T_I.reconf()
}
...
D.onfullscreenchange = function(e) {
    if (D.fullscreenElement) togglesimple(0);
    else togglesimple(1)
}
popmsgTO = F
popmsgexitTO = F
...
function animate_if_onscreen() {
    for (var i in animatable) {
        ...
        if (o) {
            r = o.getBoundingClientRect(), cn = o.className.replace(/ *onscreen/, '').replace(/ *offscreen/, ''), m = Math.min(300, wh * .7)
...
function toggle_faq(p) {
    var o = p.parentElement,
        h = '0',
        cln = 'closed',
        dw = o.getElementsByClassName('faq_answer_wrapper')[0]
...
function show_datechooser(q) {
    var x = 'datechooser',
        o = makediv(x, x + ' popc show')
    setpoppos(q, x, 0, 0, 680)
    render_cal(x, 0)
}

function close_datechooser() {
    var o = gob('datechooser');
    if (o) {
        o.className = 'hide'
    }
}

function set_inpv(o, i) {
    var c = i.split('_');
    if (3 < c.length) o.value = sh_d(new Date(Date.UTC(c[1], c[2] - 1, c[3])))
}

function goto_this_year() {
    if (!gob('calendar_body')) return;
    calY = todayY;
    calM = 1;
    render_cal('calendar_body', 0)
}

function expand_favs() {
    gob('favs').className = 'tbx';
    gob('favsXpandr').style.display = 'none';
    return F
}


function update_clock_and_date() {
    current_time = new Date();
    // 24 hour clock
    var hours = current_time.getHours();
    var minutes = current_time.getMinutes();
    var seconds = current_time.getSeconds();
    first_hours_digit = Math.floor(hours / 10);
    second_hours_digit = hours % 10;
    first_minutes_digit = Math.floor(minutes / 10);
    second_minutes_digit = minutes % 10;
    first_seconds_digit = Math.floor(seconds / 10);
    second_seconds_digit = seconds % 10;
    // Update the clock
    document.getElementById('bcdigit1').innerHTML = first_hours_digit;
    document.getElementById('bcdigit2').innerHTML = second_hours_digit;
    document.getElementById('bcdigit3').innerHTML = first_minutes_digit;
    document.getElementById('bcdigit4').innerHTML = second_minutes_digit;
    document.getElementById('bcdigit5').innerHTML = first_seconds_digit;
    document.getElementById('bcdigit6').innerHTML = second_seconds_digit;
    // Update the date
  
    var day = current_time.getDay();
    var day_of_month = current_time.getDate();
    ...
    var day_name = week_days[day];
    var month_name = months[month - 1];

    // Get number of current week
    var onejan = new Date(current_time.getFullYear(), 0, 1);
    var millisecsInDay = 86400000;
    var dayOfYear = ((current_time - onejan) / millisecsInDay) + 1;
    var week_number = Math.ceil(dayOfYear / 7);
    

    var date_string = day_name + ', ' + month_name + ' ' + day_of_month + ', ' + year + ', <span class="nw">Semana ' + week_number + '</span>';

    document.getElementById('dd').innerHTML = date_string;
